...
export const BreadCrumbs = () => {
    ...
    return (
        <div className="breadcrumbs-wrapper">
            <div className="path-start"><PathTcrIcon style={{ marginLeft: '1.5rem' }} className='icons' /><span>tcr:</span></div>
            {pathname.includes('projectspace') &&
                <div className="path-projects">
                    <PathProjectsIcon className='icons' /><span>
                        <Link to={`/projectspace`} >{loggedInUser} @ Projects</Link></span>
                </div>}
            {pathname.includes('projectspace/project') &&
                <div className="path-project-file">
                    <PathFileIcon className='icons' /><span><Link to={`/projectspace/project/${project?._id}`}>{project?.title}</Link></span>
                </div>}
            {pathname.includes('/cr') &&
                <div className="path-project-cr">
                    <PathCRIcon className='icons' /><span>democr</span>
                </div>}
        </div>
    )
}